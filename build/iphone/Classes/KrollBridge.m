...
NSString * ProgrammingAssignment3$ModuleRequireFormat = @"(function(exports){"
...
@implementation ProgrammingAssignment3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_programmingassignment3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create ProgrammingAssignment3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "ProgrammingAssignment3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_programmingassignment3 = [[ProgrammingAssignment3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_programmingassignment3];
	
	NSString *_programmingassignment3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _programmingassignment3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_programmingassignment3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:ProgrammingAssignment3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.programmingassignment3.kroll" 
